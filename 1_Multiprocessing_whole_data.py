...
file = '/media/hannah/UNTITLED/Warner_2015/201501 - part-m-000.gz'
data =file
# import warnings
...
def main():
    ...
    for chunk in raw_data:
        ...
        multiprocess_func(object_func=data_batch, name_list=parameter_tuple, set_sleep_time=set_sleep_time,
                          ...
                          cpu_max=cpu_max)
        np.save('./return_dict_1/return_dict_' + str(chunk_no) + '.npy', return_dict.copy())
        print(chunk_no, "{:.2f} seconds".format(time.time() - t_ref), 'saved retrun_dict to ./return_dict_1/return_dict_' + str(chunk_no) + '.npy')
        chunk_no+=1
